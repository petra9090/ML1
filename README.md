...
| 2 | Poisson GLM | Stay Length | `glm(family = quasipoisson)` |
...
| 5 | Support Vector Machine | Cancellation (interactions) | `kernlab::ksvm()` |
| 6 | Neural Network | Cancellation (flexible) | `nnet::nnet()` |

All models trained on 80% of the data and evaluated on the held-out 20%.
...
├── report_final.Rmd            # Main analysis — knit this to reproduce
├── report_final.html           # Rendered HTML report (submission ready)
├── README.md
└── ML1.Rproj
```
...
3. Knit `report_final.Rmd` to HTML
...
**Required R packages:**
`tidyverse`, `readr`, `httr`, `jsonlite`, `lubridate`, `car`, `multcomp`, `broom`, `kableExtra`, `scales`, `patchwork`, `caret`, `pROC`, `mgcv`, `kernlab`, `PRROC`, `nnet`
